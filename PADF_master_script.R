...
if (Sys.getenv("GITHUB_ACTIONS") == "true") {
  ...
  id_24 <- Sys.getenv("ID24")
  id_alertas <- Sys.getenv("IDALERTAS")
  id_keys <- Sys.getenv("IDKEYS")
  
  
  # Validación temprana
  if (any(vapply(c(server, username, password, email, formid, creds, id_24, id_alertas, id_keys), is_blank, logical(1)))) {
  ...
  if (file.exists(".env")) {
    ...
    if (any(vapply(c(server, username, password, email, formid, temp_creds_file, id_24, id_alertas, id_keys), is_blank, logical(1)))) {
...
load_script("Import_Data_PADF.R")
load_script("Alertas_PADF.R")
load_script("Export_data_PADF.R")
